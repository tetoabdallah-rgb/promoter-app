...
/* Offline persistence disabled to prevent query hanging on login */
...
async function uploadProductsExcel() {
    let comp = $('prodCompany').value.trim(), fileInput = $('prodFile');
...
async function loadAdminProducts() {
    ...
    tb.innerHTML = `<tr><th>${t('company')}</th><th>${t('sale_code')}</th><th>${t('sale_desc')}</th><th>${t('sale_price')}</th><th>${t('action')}</th></tr>`;
    let snap = await db.collection('products').orderBy('timestamp', 'desc').limit(50).get();
    snap.docs.forEach(doc => {
        let p = doc.data(); tb.innerHTML += `<tr><td>${p.company}</td><td>${p.itemCode}</td><td>${p.description}</td><td>${p.price}</td><td><button class="btn btn-danger" style="padding:4px;" onclick="db.collection('products').doc('${doc.id}').delete().then(loadAdminProducts)">✖️</button></td></tr>`;
    });
}
async function uploadStockExcel() {
    let comp = $('stockCompany').value.trim(), fileInput = $('stockFile');
    if(!comp || !fileInput.files.length) return toast(t('fill_fields'), 'error');
    $('loader').classList.remove('hidden');
    let reader = new FileReader();
    reader.onload = async function(e) {
        try {
            ...
            for(let i = 1; i < json.length; i++) {
                ...
                if(row.length > 0) {
                    ...
                    batch.set(docRef, { company: comp, code: row[0]||'', name: row[1]||'', price: row[2]||0, quantity: row[3]||0 });
                }
            }
            await batch.commit();
            toast(t('upload_success'), 'success'); $('stockCompany').value = ''; fileInput.value = '';
            loadAdminStock();
        } catch(err) { toast('Error parsing Excel: '+err.message, 'error'); }
        $('loader').classList.add('hidden');
    };
    reader.readAsArrayBuffer(fileInput.files[0]);
}
