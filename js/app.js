...
async function initPromoterSales() {
    $('loader').classList.remove('hidden');
    try {
        let snap = await db.collection('company_stock').where('company', '==', userData.company).where('adminId', '==', userData.adminId || currentUser.uid).get();
        cachedProducts = snap.docs.map(d => ({id: d.id, ...d.data()}));
        let select = $('sCode');
        if(select) {
            select.innerHTML = `<option value="">اختر الصنف</option>`;
            cachedProducts.forEach(p => { select.innerHTML += `<option value="${p.code}">${p.code} - ${p.name}</option>`; });
        }
        loadMySales();
        ...
        try {
            ...
            for(let i = 1; i < json.length; i++) {
                ...
                if(row.length > 0) {
                    ...
                    batch.set(docRef, { company: comp, code: String(row[0]||''), name: String(row[1]||''), price: Number(row[2]||0), quantity: Number(row[3]||0), adminId: currentUser.uid });
                }
            }
            await batch.commit();
